...
def add_to_notion(district, dong_name, apt_name, area, price, floor, deal_date, household_count=None):
    ...
    properties = {
        ...
        "동이름": {"rich_text": [{"text": {"content": dong_name}}]},
    ...
    try:
        ...
        if res.status_code == 200:
            h_text = f" | {household_count:,}세대" if household_count else " | 세대수 미확인"
            print(f"✅ [노션 등록 성공] {full_apt_name} ({dong_name}) | {area}m² | {price:,}만원 | {floor}층{h_text} | {deal_date}")
        else:
...
def get_household_count(district, dong_name, apt_name):
    ...
    try:
        search_url = f"https://search.naver.com/search.naver?query={urllib.parse.quote(district + ' ' + dong_name + ' ' + clean_apt + ' 세대수')}"
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
        res = requests.get(search_url, headers=headers, timeout=5)
        if res.status_code == 200:
            match = re.search(r'([0-9,]+)\s*세대', res.text)
            if match:
...
def fetch_and_sync_real_price():
    ...
    # 매월 직접 변경할 필요 없이 실행하는 현재 달(YYYYMM) 데이터를 자동으로 추출
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    apt_name = apt_element.text.strip()
                    dong_name = dong_element.text.strip() if dong_element is not None else ""
